// Title filters. These run before a single token is spent, so a bug here is
// either wasted money (too loose) or a job you never see (too tight).
//
// SEPARATE from filters.ts on purpose, and it is not tidiness. filters.ts
// reaches geo.ts, which carries a generated 284KB postal gazetteer. Ingest
// needs the TITLE rules — it crawls employers already chosen for being in
// range, so what it must reject is a warehouse job, not a distant one — and
// making it drag the whole gazetteer into its deploy bundle for that is how
// the discovery function got killed with WORKER_RESOURCE_LIMIT.
//
